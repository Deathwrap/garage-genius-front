...
export default {
  ...
  methods: {
    async fetchCategories() {
      try {
        const response = await axios.get('http://25.35.30.37:5198/api/services/categories'); // Замените 'API_URL' на ваш URL для запроса категорий услуг
    ...
    async fetchServicesForCategories() {
      for (const category of this.categories) {
        try {
          const response = await axios.get(`http://25.35.30.37:5198/api/services/by-id?categoryId=${category.id}`); // Замените 'API_URL' на ваш URL для запроса услуг
